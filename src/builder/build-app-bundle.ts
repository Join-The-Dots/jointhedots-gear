...
export function create_bundle_target(opts: {
   ...
   target.esmodules.plugins.push({
      ...
      resolveId(source, importer) {
         // Handle external-bundle-proxy namespace
         if (source.startsWith('\0external-bundle-proxy:')) {
            return { id: source, external: true }
         }

         let state = paths_qualifier.check(source)
         if (state === PathStatus.Dependency || state === PathStatus.External) {
            //target.log.info(`External: ${source} <- ${importer}`)
            return { id: source, external: true }
         }
         if (state === PathStatus.ExternalBundle) {
            throw new Error(`ExternalBundle: ${source} <- ${importer}`)
         }
         if (state instanceof Bundle) {
            //target.log.info(`Bundle: ${state.id} <- ${importer}`)
            const entry_id = state.resolve_export(source)
            if (!entry_id) {
               this.error(`Bundle '${state.id}' do not distribute expected entry: ${source}`)
               return null
            }
            ...
            const externalBundleModuleId = `\0external-bundle-proxy:${source}_proxy.cjs`
            ...
            return { id: externalBundleModuleId }
         }
         //target.log.trace(`Internal: ${source} <- ${importer}`)
         return null
      },
      load(id) {
         // Load virtual proxy modules for external bundle references
         if (id.startsWith('\0external-bundle-proxy:')) {
            const contents = externalBundleModules.get(id)
            if (!contents) {
